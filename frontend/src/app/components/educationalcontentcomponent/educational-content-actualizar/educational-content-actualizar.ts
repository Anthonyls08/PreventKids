...
export class EducationalContentActualizar implements OnInit {
  ...
  aceptar() {
    if (this.form.valid) {
      this.contenido.idEducationalContent = this.form.value.codigo;
      ...
      this.eS.update(this.contenido).subscribe({
        next: () => {
  ...
  init() {
    this.eS.listId(this.id).subscribe((data) => {
      this.form.patchValue({
        codigo: data.idEducationalContent,
        tittleEducationalContent: data.tittleEducationalContent,
        descriptionEC: data.descriptionEC,
        urlContent: data.urlContent,
        typeEC: data.typeEC,
        idProfessionalProfile: data.idProfessionalProfile,
        idTipocontenido: data.idTipocontenido,
      });
    });
  }
}
