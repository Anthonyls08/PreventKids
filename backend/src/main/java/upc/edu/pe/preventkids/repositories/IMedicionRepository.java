...
public interface IMedicionRepository extends JpaRepository<Medicion,Integer> {
    // DECISIÓN: mediciones con IMC alto (soesgo primero
...
ORDER BY m.imc DESC")
    ...
    @Query("SELECT m FROM Medicion m WHERE m m.temperatura >= :temperaturaMaxima")
    List<Medicion> decidirEvaluacionSignosVitales(@Param("presionMaxima") float presionMaxima,
                                            ma") float temperaturaMaxima);

    // DECISIÓN: mediciones con riesgo nutril IMC.
    // Si clasificacion viene vacia se listan ambos niveles de riesgo.
    @Query("SELECT m FROM Medicion m WHERE meso', 'Obesidad') " +
            "AND (:clasificacion = '' OR m.clasificacionimc = :clasificacion)")
    List<Medicion> decidirRiesgoNutricional(g clasificacion);

    // FILTRO: mediciones de un hijo especif
    @Query("SELECT m FROM Medicion m WHERE m.hijo.idHijo = :idHijo")
    List<Medicion> filtrarPorHijo(@Param("id
}               