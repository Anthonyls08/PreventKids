...
public interface IMedicionRepository extends JpaRepository<Medicion,Integer> {
    // Mayor riesgo primero: se atiende antes al IMC mas alto
    @Query("SELECT m FROM Medicion m WHERE m.imc >= :imcMinimo AND m.clasificacionimc = 'Sobrepeso' ORDER BY m.imc DESC")
